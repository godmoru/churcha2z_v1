...
window.webcam = {
	...
	swf_url: "/employee/public/incs/js/webcam/webcam.swf", // URI to webcam.swf movie (defaults to cwd)
	shutter_url: '/employee/public/incs/js/webcam/shutter.mp3', // URI to shutter.mp3 sound
	...
	snap: function(url, callback, stealth) {
		...
		// this.get_movie().snap();
		this.get_movie().snap( this.api_url, this.quality, this.shutter_sound ? 1 : 0, this.stealth ? 1 : 0 );
	},
	
	freeze: function() {
		// freeze webcam image (capture but do not upload)
		this.get_movie().snap('', this.quality, this.shutter_sound ? 1 : 0, 0 );
	},
	...
	set_shutter_sound: function(enabled, url) {
		...
		this.shutter_url = url ? url : '/employee/public/incs/js/webcam/shutter.mp3';
	},
