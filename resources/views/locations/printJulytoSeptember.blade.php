...
<html>
    <head>
        ...
        <title>{{ config('app.name', 'DIGC eManager') }} | {{isset($pageName) ? $pageName : 'Locations\' summary Report for July - September, '. date('Y')}}</title>
        <!-- CSS -->
    ...
    <body onclick="window.print();"> 
        ...
        <p class="h5 text-uppercase" align="center" style="font-family: Arial; font-weight: bold; font-size: 18px;">DUNAMIS INTERNATIONAL GOSPEL CENTER <br>{{'locations\' Summmary report for July - September, '. date('Y')}}  </p>
    </div>
    <div class="row">
        <div class="col-md-12">
            <table id="summary" border="2" style="border-style: all;" class="table table-hover" data-options='{ "paging": false; "searching":false}'>
                <thead>
                    ...
                    <tr>
                        <td width="7%" style="text-align: center; text-transform: uppercase; font-weight: bold; font-size: 14px;">JULY</td>
                        <td width="7%" style="text-align: center; text-transform: uppercase; font-weight: bold; font-size: 14px;">AUGUST</td>
                        <td width="7%" style="text-align: center; text-transform: uppercase; font-weight: bold; font-size: 14px;">SEPTEMBER</td>
                ...
                <tbody>
                    ...
                    <tr style="border-style: all; border-width: 2px;">
                        ...
                        <td align="center" style="font-weight: bold; font-size: 14px;"> <!-- July Average-->
                            {{
                                number_format((
                        ...
                        <td align="center" style="font-weight: bold; font-size: 14px;"> <!-- August Average-->
                            {{
                                number_format((
                        ...
                        <td align="center" style="font-weight: bold; font-size: 14px;"> <!-- Sept Average-->
                            {{
                                number_format((
                        ...
                        <td align="center" style="font-weight: bold; font-size: 14px;"> <!-- Quarter's Average-->
                            {{
                               ...
                               \App\WeeklyReport::where('service_type_id', '!=',5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 7)->where('location_id', $location->id)->sum('attendance_male') +
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 7)->where('location_id', $location->id)->sum('attendance_female') +
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 7)->where('location_id', $location->id)->sum('attendance_children'))/5 +

                                (\App\WeeklyReport::where('service_type_id', '!=',5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 8)->where('location_id', $location->id)->sum('attendance_male') +
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 8)->where('location_id', $location->id)->sum('attendance_female') +
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 8)->where('location_id', $location->id)->sum('attendance_children'))/4 +

                                (\App\WeeklyReport::where('service_type_id', '!=',5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 9)->where('location_id', $location->id)->sum('attendance_male') +
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 9)->where('location_id', $location->id)->sum('attendance_female') +
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 9)->where('location_id', $location->id)->sum('attendance_children'))/5)
                               /3, 0)
                    ...
                    <tr>
                        ...
                        <td align="center" style="font-weight: bold; font-size: 14px;"> <!-- Sept's Average-->
                            {{
                                number_format((
                                \App\WeeklyReport::where('service_type_id',5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 9)->where('location_id', $location->id)->sum('attendance_male') +
                                \App\WeeklyReport::where('service_type_id', 5)->where('year', \Carbon\Carbon::now()->format('Y'))->where('month', 9)->where('location_id', $location->id)->sum('attendance_female') +
