...
<div class="row my-3 ">
        ...
        <div class=" card b-0">
            <div class="card-header white">
                ...
                <div class="pull-right" align="right">
                	<b class="text-uppercase"> Records for {{ $datefrom }} by count sections</b> <a data-toggle="modal" data-target="#dategetATT" class="btn btn-xs btn-success"> <i class="icon-pencil"></i> Get By Date/Service</a>
                </div>
            </div>
            <div class="card-body">
				...
				<table id="example2" class="table table-bordered table-hover data-tables" data-options='{ "paging": false; "searching":false}'>
					...
					<tbody>
						...
						@foreach($attendances as $count => $aTT)
						    <td>{{ ++$count }}</td>  
						    <td><a href=""> <b>{{ strtoupper(isset($aTT->countArea) ? $aTT->countArea->name: "Undefined") }}</b></a></td>
						    ...
						    <td align="center" width="4%"><a data-male="{{$aTT->male}}" data-female="{{$aTT->female}}" data-children="{{$aTT->children}}"  data-id="{{$aTT->id}}" data-datefrom="{{$datefrom}}" data-toggle="modal" data-target="#editAttRecordx" class="btn btn-xs btn-warning orange accent-1"> <i class="icon icon-pencil"></i>Edit</a></td>
	...
	@include('locations.modals.editattendancerecordx')
