...
<html>
    <head>
        ...
        <title>{{ config('app.name', 'DIGC eManager') }} | {{isset($pageName) ? $pageName : ' Quarterly Report for - '. date(' Y')}}</title>
        <!-- CSS -->
    ...
    <body onclick="window.print();"> 
        ...
        <p class="h5 text-uppercase" align="center" style="font-family: Arial; font-weight: bold;">DUNAMIS INTERNATIONAL GOSPEL CENTER <br>Locations' Quarterly Report {{date('Y')}} </p>
    </div>
    <div class="row">
        <div class="col-md-12">
            <table id="summary" border="2" style="border-style: all;" class="table table-hover" data-options='{ "paging": false; "searching":false}'>
                <thead>
                    <tr style="border-width: 1px;">
                        ...
                        <td colspan="" style="text-align: center; text-transform: uppercase; font-weight: bold; font-size: 12px;">1st Quarter</td>
                        <td colspan="" style="text-align: center; text-transform: uppercase; font-weight: bold; font-size: 12px;">2nd Quarter</td>
                        <td colspan="" style="text-align: center; text-transform: uppercase; font-weight: bold; font-size: 12px;">3rd Quarter</td>
                        <td colspan="" style="text-align: center; text-transform: uppercase; font-weight: bold; font-size: 12px;">4th Quarter</td>
                    </tr>
                    <tr>
                        <td width="7%" style="text-align: center; text-transform: uppercase; font-weight: bold; font-size: 12px;">AVG</td>
                ...
                <tbody>
                    ...
                    <tr style="border-style: all; border-width: 2px;">
                        <td width="15%" rowspan="2" style="text-transform: uppercase; font-weight: bold; font-size: 11px;"><br>{{++$numb .'. '.$location->loc_name}}</td>
                        ...
                        <td align="center" style="font-weight: bold;"> <!-- 1st Quarter Average -->
                            {{
                                number_format(((
                        ...
                        <td align="center" style="font-weight: bold;"> <!-- 2nd Quarter Average -->
                            {{
                                number_format(((
                               \App\WeeklyReport::where('service_type_id', '!=',5)->where('month', 4)->where('location_id', $location->id)->sum('attendance_male') +
                                ...
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('month', 4)->where('location_id', $location->id)->sum('attendance_children'))/5 +
                                ...
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('month', 5)->where('location_id', $location->id)->sum('attendance_children'))/4 +
                                ...
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('month', 6)->where('location_id', $location->id)->sum('attendance_children'))/5)
                               /3, 0)
                                
                            }} 
                        </td>

                        <td align="center" style="font-weight: bold;"> <!-- 3rd Quarter Average -->
                           {{
                                number_format(((
                               \App\WeeklyReport::where('service_type_id', '!=',5)->where('month', 7)->where('location_id', $location->id)->sum('attendance_male') +
                                ...
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('month', 7)->where('location_id', $location->id)->sum('attendance_children'))/5 +
                                ...
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('month', 8)->where('location_id', $location->id)->sum('attendance_children'))/4 +
                                ...
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('month', 9)->where('location_id', $location->id)->sum('attendance_children'))/5)
                               /3, 0)
                                
                            }} 
                        </td>

                        <td align="center" style="font-weight: bold;"> <!-- 4th Quarter Average -->
                            {{
                                number_format(((
                               \App\WeeklyReport::where('service_type_id', '!=',5)->where('month', 10)->where('location_id', $location->id)->sum('attendance_male') +
                                ...
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('month', 10)->where('location_id', $location->id)->sum('attendance_children'))/5 +
                                ...
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('month', 11)->where('location_id', $location->id)->sum('attendance_children'))/4 +
                                ...
                                \App\WeeklyReport::where('service_type_id', '!=', 5)->where('month', 12)->where('location_id', $location->id)->sum('attendance_children'))/5)
                               /3, 0)
