	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title text-center text-uppercase" id="myModalLabel">Add role to {{ $user->surname}}</h4>
			</div>
			<div class="modal-body">
				<form role="form" method="POST" action="{{ route('user.addrole') }}">
					{{ csrf_field() }}
					<div class="row">
					<input type="hidden" placeholder="" required="" class="form-control" name="user_id" value="{{ $user->id}}"/>
						<div class="col-md-12">
							<div class="form-group">
								<label>
									Select Desired Role(s) for {{ $user->surname}} <span class="symbol required"></span>
								</label>
